...
const buildBasicResolvers = (cacheKey, viewName) => {
    const getById = async(root, args, ctx) => {
        ...
        // }
        // else {
            const promise = ctx.db.query(`select * from ${viewName} where id = :id and "companyId" = :companyId`, {id:args.id, companyId:ctx.cid});
    ...
    const getCollection = async (root, args, ctx) => {
        ...
        const collectionPromise = ctx.db.query(`select * from ${viewName} where "companyId" = :companyId`, {companyId:ctx.cid});
